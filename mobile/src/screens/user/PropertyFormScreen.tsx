...
export default function PropertyFormScreen({ route, navigation }: any) {
  ...
  const geocodeAddress = async (address: string, city: string) => {
    const query = [address, city, "México"].filter(Boolean).join(", ");
  ...
  const onSubmit = async () => {
    ...
    let latitud = form.latitud.trim();
    let longitud = form.longitud.trim();

    if (!latitud || !longitud) {
      setSaving(true);
      const location = await geocodeAddress(form.direccion.trim(), form.ciudad.trim());
      setSaving(false);

      if (!location) {
        Alert.alert(
          "Ubicación requerida",
          "No pude ubicar esa dirección automáticamente. Selecciona el punto en el mapa para que los compradores puedan verlo."
        );
        return;
      }

      latitud = String(location.lat);
      longitud = String(location.lng);
      setForm((prev) => ({
        ...prev,
        latitud,
        longitud,
    ...
    const payload = {
      ...
      latitud: latitud ? parseNumber(latitud) : null,
      longitud: longitud ? parseNumber(longitud) : null,
      telefono_contacto: "",
    };
    ...
    if (payload.latitud === null && latitud !== "") {
      Alert.alert("Dato inválido", "La latitud debe ser numérica.");
      return;
    }

    if (payload.longitud === null && longitud !== "") {
